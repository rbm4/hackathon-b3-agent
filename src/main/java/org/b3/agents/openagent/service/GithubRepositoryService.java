...
public class GithubRepositoryService {
    ...
    public void crawlRepositories() {
        ...
        for (var repo : repos) {
            ...
            try {
                var apiUrl = repo.getUrl().replace("https://github.com/", "https://api.github.com/repos/");
                // Get default branch
                ...
                if (responseCode == 200) {
                    ...
                    is.close();
                    var defaultBranch = json.split("\"default_branch\":\"")[1].split("\"")[0];
    ...
    private GithubTreeResponseDTO getRepositoryTreeRecursive(String apiUrl, String sha, GithubRepository repo,
            RepositoryFile parentFile)
            throws IOException {
        var treeApiUrl = apiUrl + "/git/trees/" + sha;
        var treeConn = GithubApiUtils.openAuthenticatedConnection(treeApiUrl);
        if (treeConn.getResponseCode() == 200) {
            var treeIs = treeConn.getInputStream();
            var treeJson = new String(treeIs.readAllBytes());
            treeIs.close();
            var treeDTO = new Gson().fromJson(treeJson, GithubTreeResponseDTO.class);
            if (treeDTO.getTree() != null) {
                for (var entry : treeDTO.getTree()) {
                    if ("tree".equals(entry.getType())) {
                        // Recursively fetch children for subtrees, and atribute this tree as parent
                        var parentTree = saveTree(repo, entry);
                        entry.setChildren(
                                getRepositoryTreeRecursive(apiUrl, entry.getSha(), repo, parentTree).getTree());
                    }
                    if ("blob".equals(entry.getType())) {
                        ...
                        // repo);
                        getBlobContent(entry.getUrl(), repo, entry.getPath(), parentFile);
                    }
                }
            }
            return treeDTO;
    ...
    private RepositoryFile saveTree(GithubRepository repo, GithubTreeResponseDTO.TreeEntry entry) {
        var fileEntity = fileRepository.findByRepositoryAndFilePath(repo, entry.getPath())
                .orElse(new RepositoryFile());
        fileEntity.setFileName(entry.getPath());
        fileEntity.setRepository(repo);
        fileEntity.setType(RepositoryFileTypeEnum.TREE);
    ...
    private void getRepositoryTree(String apiUrl, String branch, GithubRepository repo)
        ...
        var branchApiUrl = apiUrl + "/branches/" + branch;
        var branchConn = GithubApiUtils.openAuthenticatedConnection(branchApiUrl);
        if (branchConn.getResponseCode() == 200) {
            var is = branchConn.getInputStream();
            var json = new String(is.readAllBytes());
            is.close();
            ...
            var fullTree = getRepositoryTreeRecursive(apiUrl, treeUrlExtract, repo, null);
    ...
    private void getBlobContent(String url, GithubRepository repo, String fileName, RepositoryFile parent)
            throws IOException {
        var contentConn = GithubApiUtils.openAuthenticatedConnection(url);
        if (contentConn.getResponseCode() == 200) {
            var contentIs = contentConn.getInputStream();
            var contentBlob = new String(contentIs.readAllBytes());
            var blob = new Gson().fromJson(contentBlob, GithubBlobResponseDTO.class);
            var content = blob.getContent().replaceAll("\\s+", "");
            var decodedContent = Base64.getDecoder().decode(content);
            var fileContent = new String(decodedContent, StandardCharsets.UTF_8);
            ...
            fileEntity.setType(RepositoryFileTypeEnum.BLOB);
    ...
    public List<FileDocumentationDTO> generateDocumentation(Long id) {
        var repositoryEntity = repository.findById(id).orElseThrow();
        var ret = fileRepository.findByRepository(repositoryEntity);
        ...
        for (RepositoryFile file : ret) {
            if (StringUtils.hasText(file.getDocumentation())) {
                try {
                    ...
                    json.setFileName(buildFileName(file));
                    ...
                    fixed = fixed.replaceAll("\"\s*\"([a-zA-Z_][a-zA-Z0-9_]*?)\"\s*:", "\",\"buildFileName\":");
                    ...
                    fixed = fixed.replaceAll("(\\d)\\s*\"([a-zA-Z_][a-zA-Z0-9_]*?)\"\\s*:", "buildFileName,\"private\":");

                    // Fix missing commas between boolean values and next keys
                    // Pattern: true"key": -> true,"key":
                    fixed = fixed.replaceAll("(true|false)\s*\"([a-zA-Z_][a-zA-Z0-9_]*?)\"\s*:", "buildFileName,\"private\":");

                    // Fix missing commas between null values and next keys
                    // Pattern: null"key": -> null,"key":
                    fixed = fixed.replaceAll("(null)\s*\"([a-zA-Z_][a-zA-Z0-9_]*?)\"\s*:", "buildFileName,\"private\":");
                    ...
                    // Fix missing commas between string values inside arrays: ["value" "value"] ->
                    // ["value","value"]
    ...
    private String buildFileName(RepositoryFile file) {
        if (file.getParent() != null) {
            return buildFileName(file.getParent()) + "/" + file.getFileName();
        }
 ...
 * java.net.URL("https://ki6.com.br/hackathon_b3/documentacao");
 ...
 * var rets = response.replaceAll("```json", "").replaceAll("```",
 * "").replaceAll("\\n", "").trim();
